import { pathtracing_boundingbox_intersect, pathtracing_box_intersect, pathtracing_bvhTriangle_intersect, pathtracing_calc_fresnel_reflectance, pathtracing_plane_intersect, pathtracing_random_functions, pathtracing_uniforms_and_defines } from './commonShaderFragments';
...
export default glsl(`
  ...
  uniform mat3 uAlbedoTexturesMatrices[8];
	...
	${pathtracing_random_functions}
	${pathtracing_calc_fresnel_reflectance}
	${pathtracing_plane_intersect}
	${pathtracing_box_intersect}
	${pathtracing_boundingbox_intersect}
	${pathtracing_bvhTriangle_intersect}
    ...
    bool isRayExiting = false;
      ...
      hitUV = ( uAlbedoTexturesMatrices[0] * vec3( hitUV, 1 ) ).xy;
	...
	//-----------------------------------------------------------------------
	{
      ...
      {
        hitColor = texture(tAlbedoTextures[0], hitUV).rgb;
        ...
        //}
			...
			if (hitType == DIFF || hitType == CHECK) // Ideal DIFFUSE reflection
			{
				if (hitType == CHECK)
				{
					vec3 checkCol0 = vec3(0.8);
					vec3 checkCol1 = vec3(0.2);
					float q = clamp( mod( dot( floor(x.xz * 0.4), vec2(1.0) ), 2.0 ) , 0.0, 1.0 );
	...
	void main( void )
	{
		...
		// Add ground plane
		plane = Plane( vec4(0, 1, 0, 0.0), vec3(0), vec3(1.0, 0.5, 0.0), CHECK);
